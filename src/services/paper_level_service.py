...
async def create_paper_qna(project_id):
    ...
    for file in os.listdir(f"data/{project_id}/files"):
        if file.endswith(".pdf"):
            ...
            for i, chunk in enumerate(chunking(text, model_params["model"])):
                logger.info(f"Creating paper-level Q&A for {file} - {i}")
    ...
    for file, summary in summary_result.items():
        with open(
            f"data/{project_id}/paper-level/{file.replace('.pdf', '.json')}", "w"
        ) as f:
